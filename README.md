...
- Diagnostic local (Ollama / Gemma / Edge).
...
2. Lancer **Diagnostic local** (recommandé au démarrage poste).
3. Choisir le type de ticket (**incident** ou **demande**).
4. Utiliser soit les boutons manuels, soit **Décrocher (auto)** / **Raccrocher (auto)**.
5. Si mode manuel/Mitel: cliquer **Transcrire** puis **Analyser**.
6. Vérifier et corriger le ticket dans le formulaire.
7. **Exporter JSON** (optionnel).
8. **Ouvrir / Remplir le helpdesk**.
9. Vérifier dans le navigateur.
10. Cliquer manuellement sur **Enregistrer** dans Isilog.
11. Cliquer **Valider ticket final** dans l’app pour apprentissage local.
...
## Vérification avant test production
Lancer le pré-check local complet:
```powershell
.\.venv\Scripts\activate
...
- chargement du mapping catégories,
- disponibilité locale d'Ollama/Gemma/Edge (warning explicite si service indisponible).
...
- Connexion native au bouton "Décrocher" Mitel via API/SDK (si accessible côté poste).
- Préremplissage robuste des composants Isilog avancés (grilles, combos custom JS).
- Historique agent avec recherche plein texte des tickets et transcriptions.
- Mode supervision qualité (contrôles champs obligatoires avant préremplissage).
